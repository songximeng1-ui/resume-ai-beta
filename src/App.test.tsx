...
test('V0.7 首页选择投递无反馈路线后复用旧无 JD 经历盘点能力', async () => {
  const user = userEvent.setup();
  window.localStorage.setItem(BETA_STORAGE_KEY, JSON.stringify({ authorized: true, betaAccessCode: 'private-beta' }));
  render(<App />);

  await user.click(screen.getByRole('button', { name: '我投了很多，但没有面试。' }));
  await user.click(screen.getByRole('button', { name: /开始 21 天陪跑/ }));

  expect(screen.getByRole('heading', { name: '先把你的材料放进来' })).toBeInTheDocument();
  expect(screen.queryByLabelText('粘贴目标岗位要求')).not.toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '投递记录' })).toBeInTheDocument();
  expect(screen.getByText(/请先整理 2-3 条最近真实投递/)).toBeInTheDocument();
  expect(screen.getAllByLabelText('岗位名称')).toHaveLength(3);
  expect(screen.getAllByLabelText('简历版本')).toHaveLength(3);
  expect(screen.getAllByLabelText('反馈状态')).toHaveLength(3);
  expect(JSON.parse(window.localStorage.getItem(SESSION_STORAGE_KEY) || '{}')).toMatchObject({
    version: 'v0.7',
...
test('V0.7 首页选择无方向路线后复用旧无 JD 经历盘点能力', async () => {
  const user = userEvent.setup();
  window.localStorage.setItem(BETA_STORAGE_KEY, JSON.stringify({ authorized: true, betaAccessCode: 'private-beta' }));
  render(<App />);

  await user.click(screen.getByRole('button', { name: '我不知道该投什么岗位。' }));
  await user.click(screen.getByRole('button', { name: /开始 21 天陪跑/ }));
...
test('投递复盘路线结果页提供无反馈记录入口，并保存恢复 records', async () => {
  const user = userEvent.setup();
  window.localStorage.setItem(BETA_STORAGE_KEY, JSON.stringify({ authorized: true, betaAccessCode: 'private-beta' }));
  window.localStorage.setItem(
    SESSION_STORAGE_KEY,
    JSON.stringify({
      ...
      route: 'applying_no_feedback',
      step: 'result',
      plan: getInitialRoutePlan('applying_no_feedback'),
      records: [],
      legacy: {
        step: 'result',
        stage: 'senior',
        ...
        assets: [],
        truthConfirmed: true,
        resumeText: '本科 市场营销，已投递用户运营、新媒体运营和市场助理。',
        jdText: '',
        jdFit: null,
        report: { ...mockReport(), mode: 'inventory', jdFit: undefined, interviews: undefined, usage: null },
        reportTask: null
  ...
  expect(screen.getByRole('heading', { name: 'V0.7 投递复盘路线' })).toBeInTheDocument();
  expect(screen.getByText(/当前无反馈诊断/)).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '今日复盘行动' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'Day 1-3 任务' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '今日记录入口' })).toBeInTheDocument();
  expect(screen.getByText('第 1 天：整理最多 3 条真实投递记录')).toBeInTheDocument();
  expect(screen.getAllByText(/最多 3 条投递记录/).length).toBeGreaterThanOrEqual(1);
  ...
  await user.type(screen.getByLabelText('这些记录来自哪里'), '来自招聘平台投递列表和自己的简历文件名。');
  await user.type(screen.getByLabelText('目前最可疑的问题线索'), '三个岗位都要求用户反馈整理，但简历里没有对应证据。');
  await user.type(screen.getByLabelText('下一次投递前先调整什么'), '先补一条用户反馈整理的真实经历表达。');
  await user.click(screen.getByRole('button', { name: '保存今日记录' }));
  ...
  expect(saved.records[0]).toMatchObject({
    ...
    day: 1,
    completionStatus: 'partly',
    outputText: '用户运营助理，7月10日投递，运营版 v1，已读无回复。',
    evidenceText: '来自招聘平台投递列表和自己的简历文件名。',
    reflectionText: '三个岗位都要求用户反馈整理，但简历里没有对应证据。',
    nextAdjustment: '先补一条用户反馈整理的真实经历表达。'
  });

  unmount();
  render(<App />);
  expect(screen.getByRole('heading', { name: '已记录的今日行动' })).toBeInTheDocument();
  expect(screen.getByText(/用户运营助理，7月10日投递/)).toBeInTheDocument();
  expect(screen.getByTestId('app-root')).not.toHaveTextContent(/你不行|你不适合|简历很差|保证 offer|一定进面|海投|每天投 50 个|provider|token|tokens|成本|模型：|API key|base URL|原始错误/i);

  await user.click(screen.getAllByRole('button', { name: '清除本次分析数据' })[0]);
  await waitFor(() => expect(window.localStorage.getItem(SESSION_STORAGE_KEY)).toBeNull());
});

test('无方向路线结果页提供岗位样本验证记录入口，并保存恢复 records', async () => {
  const user = userEvent.setup();
  window.localStorage.setItem(BETA_STORAGE_KEY, JSON.stringify({ authorized: true, betaAccessCode: 'private-beta' }));
  window.localStorage.setItem(
    SESSION_STORAGE_KEY,
    JSON.stringify({
      ...
      legacy: {
        ...
        assets: [],
        truthConfirmed: true,
        resumeText: '本科 市场营销，有社群维护和校园调研经历，还不知道该投什么。',
        jdText: '',
        jdFit: null,
        report: { ...mockReport(), mode: 'inventory', jdFit: undefined, interviews: undefined, usage: null },
        reportTask: null
  ...
  expect(screen.getByRole('heading', { name: 'V0.7 岗位样本验证路线' })).toBeInTheDocument();
  expect(screen.getByText(/当前方向状态/)).toBeInTheDocument();
  expect(screen.getByText(/真实岗位样本提醒/)).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '今日岗位验证行动' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: 'Day 1-3 任务' })).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '今日记录入口' })).toBeInTheDocument();
  expect(screen.getByText(/当前还不能下方向结论/)).toBeInTheDocument();
  expect(screen.getByText('第 1 天：找 1 个真实可搜索的初级岗位样本')).toBeInTheDocument();
  expect(screen.getAllByText(/1 个真实可搜索的初级岗位样本/).length).toBeGreaterThanOrEqual(1);
  expect(screen.getByText(/所有方向只能叫“可探索方向”/)).toBeInTheDocument();
  expect(screen.getByText(/经历材料和岗位样本验证依据库/)).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '报告摘要' })).toBeInTheDocument();

  await user.click(screen.getByRole('radio', { name: '完成一部分' }));
  await user.type(screen.getByLabelText('今天找到的真实岗位样本'), '用户运营助理，某招聘平台，JD 摘要：社群维护、用户反馈整理、活动触达。');
  await user.type(screen.getByLabelText('这个样本来自哪里'), '用户手动搜索并复制的招聘页面。');
  await user.type(screen.getByLabelText('哪些要求目前有/没有经历证据'), '有社群维护证据；用户反馈整理证据不足；活动触达暂时没有。');
  await user.type(screen.getByLabelText('下一步只做一个动作'), '继续找 1 个用户运营助理样本对比要求是否重复。');
  await user.click(screen.getByRole('button', { name: '保存今日记录' }));
  ...
  expect(saved.records[0]).toMatchObject({
    ...
    day: 1,
    completionStatus: 'partly',
    outputText: '用户运营助理，某招聘平台，JD 摘要：社群维护、用户反馈整理、活动触达。',
    evidenceText: '用户手动搜索并复制的招聘页面。',
    reflectionText: '有社群维护证据；用户反馈整理证据不足；活动触达暂时没有。',
    nextAdjustment: '继续找 1 个用户运营助理样本对比要求是否重复。'
  });

  unmount();
  render(<App />);
  expect(screen.getByRole('heading', { name: '已记录的今日行动' })).toBeInTheDocument();
  expect(screen.getByText(/用户运营助理，某招聘平台/)).toBeInTheDocument();
  expect(screen.getByTestId('app-root')).not.toHaveTextContent(/推荐职业|推荐你做|你最适合|AI 帮你选职业|你的职业方向是|你不适合|最佳职业选择|保证 offer|更容易拿 offer|provider|token|tokens|成本|模型：|API key|base URL|原始错误/i);
...
test('无岗位要求路线最后一段追问后进入经历方向诊断，并可继续生成报告', async () => {
  ...
  expect(screen.queryByRole('heading', { name: '内部质量检查' })).not.toBeInTheDocument();
  expect(screen.getByTestId('app-root')).not.toHaveTextContent(/质量分|检查状态|暂无阻断项|quality gate|schema|字段契约/i);
...
test('基础版报告在结果页展示保守交付说明', () => {
  ...
  expect(screen.getByRole('heading', { name: '保守版行动建议' })).toBeInTheDocument();
  expect(screen.getAllByText('保守版行动建议').length).toBeGreaterThanOrEqual(2);
  expect(screen.queryByText('真实 AI 诊断')).not.toBeInTheDocument();
  expect(screen.getByText(/内容基于你确认过的信息整理/)).toBeInTheDocument();
...
test('深度 AI 报告和演示结果在结果页保留各自身份标识', () => {
  ...
  const { unmount } = render(<App />);
  expect(screen.getByText('AI 深度建议')).toBeInTheDocument();
  expect(screen.queryByText('保守版行动建议')).not.toBeInTheDocument();
  unmount();
  ...
  render(<App />);
  expect(screen.getByText('演示结果')).toBeInTheDocument();
  expect(screen.queryByText('AI 深度建议')).not.toBeInTheDocument();
});
