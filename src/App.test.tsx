...
test('开始页未选择阶段和模式时不能进入下一步，选择后进入输入页', async () => {
  ...
  expect(screen.getByText('求职地图 V0.4')).toBeInTheDocument();
...
test('动态追问、JD 匹配和报告生成等待时显示较长等待提示，报告失败后提示可重试', async () => {
  ...
  await user.click(screen.getByRole('button', { name: /生成 V0.4 定制诊断报告/ }));
...
test('有 JD 模式输出证据矩阵和 V0.4 完整诊断报告', async () => {
  const user = await startV2();
...
test('生成 V0.4 诊断报告失败时在 JD 页面显示错误原因', async () => {
  vi.mocked(fetch).mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
    const url = String(input);
    const body = init?.body ? JSON.parse(String(init.body)) : {};
    if (url.endsWith('/api/ai/status')) {
