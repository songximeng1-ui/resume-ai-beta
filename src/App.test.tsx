...
function mockDigQuestions(asset?: AssetCard): DigQuestionSet {
  return {
    ...
    source: 'demo',
    questions: [
      `你这段${asset?.title || '实习经历'}里提到“${asset?.content || '教育机构新媒体运营实习'}”，我想帮你判断它能不能转成用户运营需要的能力。你当时最核心的动作是哪 1-2 个？`,
      '这段经历有没有可核实的规模、频次或周期，例如人数、社群数量、每周次数、持续多久？',
      '如果面试官追问结果或反馈，你能说明一个真实变化、产出或复盘结论吗？'
    ],
    digIntent: '我想确认这段经历里是否有用户沟通、内容整理、活动触达或数据整理证据。',
    potentialHighlight: '这段经历可能不是普通打杂，而是基础用户触达与内容执行支持。',
    answerHint: '不用写完整文章，按“对象 + 动作 + 频次 + 结果/反馈”回答即可；没有数据也没关系，可以写待核实。',
    resumePreview: '待核实：协助维护学生社群，整理公众号推文素材，支持活动触达与反馈收集。',
    encouragement: '这段经历不是没有价值，它可能体现的是执行推进和现场协调能力。我们再补一个规模或结果信息，就能判断它是否适合写进简历。'
  };
}
...
test('可以附上文本简历文件并把内容带入简历输入框', async () => {
  ...
  expect(screen.getByText(/文本文件读取成功：resume.txt/)).toBeInTheDocument();
...
test('动态追问结合具体经历和目标岗位，支持上一段、下一段、保存并继续、跳过和返回修改', async () => {
  ...
  expect(screen.getByRole('heading', { name: '这轮想帮你确认什么' })).toBeInTheDocument();
  expect(screen.getByText(/用户沟通、内容整理、活动触达或数据整理证据/)).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '可能挖出的亮点' })).toBeInTheDocument();
  expect(screen.getByText(/基础用户触达与内容执行支持/)).toBeInTheDocument();
  ...
  expect(screen.getByText(/对象 \+ 动作 \+ 频次 \+ 结果\/反馈/)).toBeInTheDocument();
  expect(screen.getByRole('heading', { name: '如果补充得出来，可能可以这样写' })).toBeInTheDocument();
  expect(screen.getByText(/待核实：协助维护学生社群/)).toBeInTheDocument();
