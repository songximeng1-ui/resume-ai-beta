...
本次完成 V0.4 “模型可以失败，但用户一定能拿到结果”的第一层兜底：
...
- `/api/ai/report` 在真实模型或 schema 修复失败且无可用深度模块时，返回 `isBasic: true` 的基础版报告，不回退到 demo。
...
后续待补：当部分深度模块已完成但后续模块失败时，可继续扩展为“已完成深度模块 + 基础模块补齐”的混合报告，同时保留继续补全深度内容的入口。
